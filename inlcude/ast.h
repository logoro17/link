...
struct ClearStmt : public Stmt {
    void print(int indent = 0) override {
        std::cout << std::string(indent, ' ') << "ClearScreen\n";
    }
};
...
struct FuncDecl : public Stmt {
    std::string name;
    std::vector<std::string> params; // [BARU] Daftar nama parameter (a, b)
    std::vector<std::unique_ptr<Stmt>> body;

    FuncDecl(const std::string& n, std::vector<std::string> p) // Constructor baru
...
struct CallStmt : public Stmt {
    std::string func;
    std::vector<std::unique_ptr<Expr>> args; // [UBAH] Dari argExpr tunggal jadi Vector

    CallStmt(const std::string& f, std::vector<std::unique_ptr<Expr>> a) 
